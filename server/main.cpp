...
class ServerConnection : public std::enable_shared_from_this<ServerConnection>, boost::noncopyable {
    ServerConnection(boost::asio::io_service &service, RoomsMap &rooms, boost::uuids::random_generator &generator,
                     Logger &logger) : sock_(service), started_(false), rooms_(rooms),
...
public:
    ...
    static ServerPointer
    create(boost::asio::io_service &service, RoomsMap &rooms, boost::uuids::random_generator &generator,
           Logger &logger) {
...
private:
                        ...
                        for (auto &elem: it->second) {
                            ...
                            if (elem->getUsername() != username_) {
                                ...
                                elem->sock().async_write_some(boost::asio::buffer(dump, dump.size()),
                                                              [shared_this = shared_from_this()](const ErrorCode &err,
                                                                                                 size_t bytes) {
                                                                  shared_this->on_write(err, bytes);
                                                              });
                            }
    ...
    void do_read() {
        async_read(sock_, boost::asio::buffer(read_buffer_),
                   ...
                   [shared_this = shared_from_this()](const boost::system::error_code &err,
                                                      std::size_t bytes) { shared_this->on_read(err, bytes); });
    }
    ...
    void do_write(const std::string &msg) {
        ...
        sock_.async_write_some(boost::asio::buffer(write_buffer_, msg.size()),
                               [shared_this = shared_from_this()](const ErrorCode &err,
                                                                  size_t bytes) { shared_this->on_write(err, bytes); });
    }
...
void handle_accept(ServerPointer client, const boost::system::error_code &err,
                   boost::asio::ip::tcp::acceptor &acceptor, boost::asio::io_service &service, RoomsMap &rooms,
                   boost::uuids::random_generator &generator, Logger &logger) {
