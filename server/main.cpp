...
class ServerConnection : public std::enable_shared_from_this<ServerConnection>, boost::noncopyable {
    ServerConnection(boost::asio::io_service& service, RoomsMap& rooms, boost::uuids::random_generator& generator, Logger& logger) : sock_(service), started_(false), rooms_(rooms),
            random_generator_(generator), logger_(logger) {}
public:
    ...
    static ServerPointer create(boost::asio::io_service& service, RoomsMap& rooms, boost::uuids::random_generator& generator, Logger& logger) {
...
void handle_accept(ServerConnection::ServerPointer client, const boost::system::error_code & err, boost::asio::ip::tcp::acceptor& acceptor, boost::asio::io_service& service, RoomsMap& rooms, boost::uuids::random_generator& generator, Logger& logger) {
    spdlog::get("logger")->info("Handling accept");
    client->start();
    ServerConnection::ServerPointer new_client = ServerConnection::create(service, rooms, generator, logger);
    acceptor.async_accept(new_client->sock(), [client = new_client, &acceptor = acceptor, &service = service,
                                        &rooms = rooms, &generator = generator, &logger = logger]
                                        (const boost::system::error_code & err){ handle_accept(client, err, acceptor, service, rooms, generator, logger);});
}
...
int main(int argc, char* argv[]) {
    ...
    ServerConnection::ServerPointer client = ServerConnection::create(service, rooms, random_generator_, logger);
    acceptor.async_accept(client->sock(), [client = client, &acceptor = acceptor, &service = service,
                                           &rooms = rooms, &generator = random_generator_, &logger = logger]
                                           (const boost::system::error_code & err){ handle_accept(client, err, acceptor, service, rooms, generator, logger);});
    service.run();
}
