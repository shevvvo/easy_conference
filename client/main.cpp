...
public:
    ...
    static ClientPtr
    create(const boost::asio::ip::tcp::endpoint &ep, std::string &username, boost::asio::io_service &service,
           Logger &logger) {
...
private:
    void on_connect(const ErrorCode &err) {
        ...
        primitives::Command opt = primitives::get_user_command(std::cin, std::cout,
    ...
    void on_create_sent(const ErrorCode &err, size_t bytes) {
        ...
        async_read(sock_, boost::asio::buffer(read_buffer_),
                   [shared_this = shared_from_this()](const boost::system::error_code &err,
                                                      size_t bytes) { return shared_this->read_complete(err, bytes); },
                   [shared_this = shared_from_this()](const ErrorCode &err, size_t bytes) {
                       shared_this->on_create_read(err, bytes);
                   });
    }
    ...
    void on_join_sent(const ErrorCode &err, size_t bytes) {
        ...
        async_read(sock_, boost::asio::buffer(read_buffer_),
                   [shared_this = shared_from_this()](const boost::system::error_code &err,
                                                      size_t bytes) { return shared_this->read_complete(err, bytes); },
                   [shared_this = shared_from_this()](const ErrorCode &err, size_t bytes) {
                       shared_this->on_join_read(err, bytes);
                   });
    }
    ...
    void read_from_input() {
        async_read(input_stream_, boost::asio::buffer(input_buffer_),
                   [shared_this = shared_from_this()](const boost::system::error_code &err,
                                                      size_t bytes) { return shared_this->input_complete(err, bytes); },
                   [shared_this = shared_from_this()](const ErrorCode &err, size_t bytes) {
